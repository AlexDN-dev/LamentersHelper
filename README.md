...
Addon WoW personnel pour le raid **The Voidspire** (Midnight 12.0).

Affiche des alertes texte (globales et/ou privées) en réponse aux mécaniques de boss détectées via le combat log.
...
| Mécanique | Type | Message affiché |
|---|---|---|
...
| Smashed ×N *(tank, stacks)* | 🌐 Global | `SMASHED ×N — [nom] — SWAP TANK !` |
| Blisterburst | 🌐 Global | `BLISTERBURST — +100% DÉGÂTS (30s) !` |
...
| Fractured Projection | 🌐 Global | `FRACTURED IMAGE INVOQUÉ — FOCUS L'ADD !` |
...
| Destabilizing Strikes ×N *(seuil, tank)* | 🌐 Global | `DESTABILIZING STRIKES ×N — [nom] — SWAP TANK !` |
...
| Diminish *(joueur ayant soak Gloom)* | 🔒 Privé | `DIMINISH — NE SOAKEZ PLUS GLOOM !` |
